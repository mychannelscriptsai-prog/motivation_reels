...
# Semaphore to allow only 1 merge at a time to save memory
merge_lock = threading.Semaphore(1)
...
def _run_ffmpeg(
    ...
    """Run FFmpeg without looping to save memory"""
    pexels_total = total_duration - cta_duration
    scene_dur = pexels_total / 3
    fade_duration = 1.0

    # Inputs: 3 scenes + CTA + audio
    inputs = [str(s) for s in scene_paths] + [str(cta_path), str(audio_path)]
    ffmpeg_inputs = []
    for i, s in enumerate(inputs[:-1]):  # last input is audio
        ffmpeg_inputs.extend(["-i", s])
    ffmpeg_inputs.extend(["-i", inputs[-1]])  # add audio
    ...
    xfade_chain = (
        f"[v0][v1]xfade=transition=fade:duration={fade_duration}:offset={scene_dur-fade_duration}[v01];"
    )
    xfade_chain += (
        f"[v01][v2]xfade=transition=fade:duration={fade_duration}:offset={scene_dur*2-fade_duration}[v02];"
    )
    ...
    af = f"[{len(scene_paths)+1}:a]volume={volume},apad[a]"

    cmd = ["ffmpeg", "-y", "-threads", "1"] + ffmpeg_inputs + [
        "-filter_complex",
        vf + xfade_chain + af,
        "-map",
        "[v]",
...
def merge(req: MergeRequest):
    """Memory-safe merge using semaphore"""
    if not merge_lock.acquire(blocking=False):
        raise HTTPException(status_code=429, detail="Another merge is running. Please try again later.")

    try:
        with tempfile.TemporaryDirectory() as td:
            td = Path(td)

            scene_paths = [td / f"scene{i}.mp4" for i in [34, 35, 37]]
            cta_path = td / "cta.mp4"
